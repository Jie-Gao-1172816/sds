...
- **Debugging and error fixing:** I asked for help when routes did not insert or update data correctly (for example, issues related to database commit/autocommit behaviour), when templates did not show expected values, and when SQL joins returned unexpected results.
...
Tools used:
- ChatGPT (AI assistant) for explanations, debugging, and also ode suggestions.
...
I used a simple black, white, and grey colour palette across the whole site. I chose these colours because they match the dance school style and make the pages look clean and professional. The neutral colours work well with the homepage dance image, and they keep tables and forms easy to read. I also kept the header and footer consistent on every page so the interface feels stable and familiar.
...
I used flash messages for the **Add Student**, **Edit Student**, and **Enrol Student** sections. This gives users clear feedback. They can see if an action was successful or if something needs to be fixed.
...
I grouped students under the same class and only displayed the class details once. This makes the class list more readable.
...
The Student Class Summary page is designed to show the student’s enrolled classes and the student’s current grade information.

During testing, I noticed a discrepancy between `studentgrades` (the student’s recorded grade) and the grade level of the class the student is enrolled in. For example, **Lucas Harris** has a mismatch between the recorded grade and the class grade. I do not know if this is test data or a mistake. To handle this clearly, I display the student’s current grades on the summary page. If a user finds a mismatch, the best solution is to edit the student grade first to match the current class grade and then enrol the correct class grade again. Users can find a description on the enrol page: if the class grade options do not match expectations, they should update the student grade first so eligible classes display correctly.

### 9. Button priority based on user goals
...
### 10. Multiple entry points for key actions
...
### 11. Date inputs with an HTML date picker
...
### 12. Preserving user input after validation errors

If a user submits a form with errors in the edit or add student sections, I re-render the form instead of redirecting. This keeps the entered values in the form so the user does not need to re-enter everything.

### 13. Enrolment requires student qualification
